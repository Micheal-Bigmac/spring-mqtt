package spring.mqtt.api.session;
...
public abstract class Session {
    ...
    public boolean removeSession(String clientId, ChannelHandlerContext session){
        if (this.repo.containsKey(clientId)) {
            this.repo.remove(clientId);
            return true;
        } else {
            return false;
        }
     ...
     * @param flush    Flush?
     */
  public void sendMessage(ChannelHandlerContext ctx, Object msg, String clientId, Integer packetId, boolean flush){
      String pid = packetId == null || packetId <= 0 ? "" : String.valueOf(packetId);
      ChannelFuture future = flush ? ctx.writeAndFlush(msg) : ctx.write(msg);
      future.addListener(f -> {
          if (f.isSuccess()) {
              logger.debug("Message succeed: Message {} {} has been sent to client {} successfully", pid, clientId);
          } else {
              logger.debug("Message failed: Message {} {} failed to send to client {}: ",  pid, clientId, f.cause());
          }
      });
  }
}
