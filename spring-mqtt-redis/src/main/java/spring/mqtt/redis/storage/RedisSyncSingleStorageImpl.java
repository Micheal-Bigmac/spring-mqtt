...
import static spring.mqtt.api.util.Topics.END;
import static spring.mqtt.redis.util.Converter.mapToMessage;
import static spring.mqtt.redis.util.Converter.messageToMap;
...
public class RedisSyncSingleStorageImpl implements SyncStorage {
    ...
    public String updateConnectedNode(String clientId, String node, int seconds) {
        ...
        String script="local old = redis.call('HGET', KEYS[1], 'node')\n" +
                "redis.call('HSET', KEYS[1], 'node', ARGV[1])\n" +
                "redis.call('EXPIRE', KEYS[1], ARGV[2])\n" +
                "return old";
        return this.script().eval(script, ScriptOutputType.VALUE, new String[]{RedisKey.connection(clientId)}, node, String.valueOf(seconds));
    }
    ...
    private void getMatchRetainMessages(List<String> topicLevels, int index, List<List<String>> list) {
        ...
        switch (level) {
            ...
            case "+":
                ...
                if (nodes != null) {
                    nodes.forEach((node, count) -> {
                        if (!node.equals(END) && Integer.parseInt(count) > 0) {
                            if (node.equals(END)) {
            ...
            default:
                ...
                if (count != null && Integer.parseInt(count) > 0) {
                    if (level.equals(END) && index == topicLevels.size() - 1) {
